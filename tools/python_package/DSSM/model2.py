...
class Dssm(Model):
    def get_config(self):
        return {"user_sparse_feature_columns": self.user_sparse_feature_columns,
                "user_dense_feature_columns": self.user_dense_feature_columns, 
...
#             'dense_' + feat['feat']:Dense(64)
#              for feat in self.user_dense_feature_columns
#         }
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        """
        user_sparse_inputs,user_dense_inputs, item_sparse_inputs = inputs
        # user tower
        # 拼接所有稀疏特征
        # embedding层输入对应特征，输出中间结果 embed[k](v)
        user_sparse_embed = tf.concat([self.user_embed_layers['embed_{}'.format(k)](v)
                                  for k, v in user_sparse_inputs.items()], axis=-1)
        
        # 拼接所有稠密特征
        for k,v in user_dense_inputs.items():
            user_dense_embed = v
        user_dense_embed = tf.cast(user_dense_embed, tf.float32)

        user_dense_embed = tf.expand_dims(user_dense_embed, axis=-1)
        
        user_dense_embed = self.dense(user_dense_embed) # dense所需最小维度为2
        
#         维度不一致不能concat,扩充维度
        
        
        # 沿着最后一维拼接
        user_dnn_input = tf.concat([user_dense_embed, user_sparse_embed], axis=-1)
#         user_dnn_input = user_sparse_embed
        
        self.user_dnn_out = self.user_dnn(user_dnn_input) # 自动调用call方法，user_dnn_input作为输入
    ...
    def build_graph(self, **kwargs):
        ...
        user_dense_inputs = {uf['feat']: Input(shape=(1, ), dtype=tf.float32) for uf in
                              self.user_dense_feature_columns}
        ...
        # 定义模型输入输出
        model = Model(inputs=[user_sparse_inputs,user_dense_inputs, item_sparse_inputs],
              outputs=self.call([user_sparse_inputs,user_dense_inputs, item_sparse_inputs]))
        
        # 参数设置
        model.__setattr__("user_input", user_sparse_inputs)
        model.__setattr__("user_dense_input", user_dense_inputs)
